...
export const PRESS_RELEASES: PressRelease[] = [
  {
    ...
    body: [
      `The six A-Tier tools are: signal_get (per-organization GitHub commit-velocity, contributor-growth, and dependent-graph deltas), signal_search (full-text search across the 400+ organization tracked panel), signal_sector (per-sector aggregations across 20 startup sectors), signal_get_deep_signal (paid agent-credit endpoint at 0.19 EUR per call for the full 47-feature signal vector), signal_trend (rolling 14-day acceleration deltas), and agent_pitch (machine-readable pitch payload for agent-to-agent integration). The first five are free with no key; the sixth is the only metered surface in the offering.`,
