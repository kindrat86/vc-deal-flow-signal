    ...
    <>
      ...
      {ga4 && (
        <Script
          ...
          strategy="afterInteractive"
          dangerouslySetInnerHTML={{
            __html: `(function(){var CONV=["signup_verify_sent","beta_signup","lead_submitted","subscribed","analysis_purchased","purchase_confirmed","lead_magnet_requested","exit_intent_subscribed","tools_subscribe_submitted"];var ENG=["concierge_opened","exit_modal_opened","exit_modal_submitted"];var EVAL_RE=/(\\/pricing|\\/vs\\/|alternatives-to|\\/methodology|\\/mcp|\\/api|\\/docs)/;var qFired=false;function pushGtag(){try{window.dataLayer=window.dataLayer||[];window.dataLayer.push(Array.prototype.slice.call(arguments));}catch(e){}}function qualified(source){var K="gdf_qualified_visit";if(qFired)return;try{if(sessionStorage.getItem(K))return;sessionStorage.setItem(K,"1");}catch(e){}qFired=true;pushGtag("event","qualified_visit",{path:location.pathname,source:source||"unknown"});}function mirror(name,props){if(!name)return;var params={source:location.pathname};if(props&&typeof props==="object"){for(var k in props){var v=props[k];if(v===null||v===undefined)continue;if(typeof v==="string"||typeof v==="number"||typeof v==="boolean")params[k]=v;}}if(ENG.indexOf(name)>=0){pushGtag("event",name,params);qualified("engagement");}else if(CONV.indexOf(name)>=0){pushGtag("event",name,params);qualified("conversion");}}function installMirror(){var ph=window.posthog;if(!ph||!ph.__loaded||typeof ph.on!=="function"||ph.__gdfMirrorListener)return;ph.on("eventCaptured",function(event){try{mirror(event&&event.event,event&&event.properties);}catch(e){}});try{ph.__gdfMirrorListener=true;}catch(e){}}if(EVAL_RE.test(location.pathname))qualified("eval_path");installMirror();var tries=0;var timer=setInterval(function(){installMirror();if(++tries>600)clearInterval(timer);},100);})();`,
          }}
        />
      )}
