...
// Canonical current public taxonomy. Keep this in lockstep with the
// conversion offer contract and do not collect intent outside these sectors.
...
/**
 * Intent capture stays separate from mailing consent. A sector choice is never
 * added to, reactivated in, or used to alter a Resend audience contact.
 */
export async function POST(req: NextRequest) {
  // Guards mirror app/api/subscribe/route.ts (audit 2026-07-18): this route
  // This route notifies the internal inbox, so it needs the same origin
  // allowlist, per-IP rate limit, and honeypot. The 7-day dedup nonce below
  // stays as the secondary guard.
  ...
  // Notify the internal inbox after the response. Intent capture is not mailing
  // consent and never adds, reactivates, or changes a Resend audience contact.
  after(async () => {
