      ...
      "/api/signals.json": {
        get: {
          ...
          description: [
            ...
            "- `sector=<slug>` → top-20 trending + the full ranked startup list for that one sector + dataset meta. ~13 KB. **Use for any sector-specific query: 'show me healthcare', 'web3 breakouts', 'gaming startups'.** Map fuzzy user input to one of the 15 slugs in `sector` enum before calling.",
        ...
        Sector: {
          ...
          properties: {
            slug: {
              ...
              enum: [...SECTOR_SLUGS],
              description:
                "Sector slug. 15 supported. Map fuzzy user input first: 'crypto'→web3, 'SaaS'→enterprise-saas, 'biotech'→healthcare, 'real estate'→proptech, 'agriculture'→agtech, 'space'→space-tech, 'games'→gaming, 'logistics'→supply-chain, 'law'→legal-tech, 'recruiting'→hr-tech, 'learning'→edtech, 'commerce'→ecommerce-infrastructure, 'hardware'→robotics, 'social'→social-community.",
            },
            name: { type: "string" },
            description: { type: "string" },
