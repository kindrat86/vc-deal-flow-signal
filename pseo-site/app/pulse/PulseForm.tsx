...
export default function PulseForm() {
  ...
  return <form onSubmit={submit} className="mt-8 space-y-6 rounded-xl border border-slate-700 bg-slate-900/60 p-6">
    <fieldset><legend className="text-sm font-medium">How likely are you to recommend GitDealFlow to another investor?</legend><div className="mt-3 grid grid-cols-6 gap-2 sm:grid-cols-11">{Array.from({ length: 11 }, (_, score) => <label key={score} className="flex min-h-10 cursor-pointer items-center justify-center rounded-md border border-slate-600 bg-slate-950 px-2 py-2 text-center"><input required type="radio" name="score" value={score} className="mr-1" />{score}</label>)}</div><p className="mt-2 text-xs text-gray-400">0 = not at all likely. 10 = extremely likely.</p></fieldset>
