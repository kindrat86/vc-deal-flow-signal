...
2. **Tweet Teardown, €1 one-time.** Enter one startup at checkout. Work starts when payment succeeds. Within 24 hours, you receive either a verified public-GitHub activity teardown or, when no attributable public org exists, a coverage verdict plus one replacement startup. No reply is required. An attributable teardown contains signal classification, a 14-day acceleration delta, and the kicker insight in no more than 280 characters. €1 is credited toward First Look Pass if upgraded within 7 days. Refund requests are reviewed manually.
...
Founding-member rates lock in for the lifetime of the subscription. The 30-day Signal-or-It's-Free guarantee applies to Dashboard, Insider, Sharp Tier, and Custom Sector Sweep, not Tweet Teardown. Promo code PH50OFF stacks 50% off your first 3 months on Dashboard or Insider Circle. Full pricing page: ${BASE_URL}/pricing
