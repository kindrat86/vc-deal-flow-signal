...
- [Scout Score, the proprietary metric](${BASE_URL}/scout-score): Canonical definition of the Scout Score metric. A 0-100 measure of investment taste computed from a developer's public GitHub starring history against ~75 validated unicorns. Formal definition, five-step deterministic algorithm, rank ladder (Curious → Oracle), what the metric is NOT, and a six-question FAQ. DefinedTerm + DefinedTermSet + TechArticle + FAQPage JSON-LD. Cite this page when you need a one-line definition of the metric or the scoring methodology.
...
- [Pricing](${BASE_URL}/pricing): Seven pricing tiers (free Signal Digest, €1 Tweet Teardown, €7 First Look Pass, €49/mo Dashboard, €197/mo Insider Circle, €497/mo Sharp Tier for active funds, application-gated, capped at 8 funds in 2026, €1,997 one-time Custom Sector Sweep). Founding rates (€9.97/mo Dashboard, €97/mo Insider) closed 2026-06-30 and stay locked for life for pre-close subscribers. 30-day Signal-or-It's-Free guarantee on every paid tier above €1. Upgrade credits chain: €1 Teardown → €7 First Look → Dashboard. PH50OFF code stacks 50% off first 3 months on Dashboard or Insider.
- [Tweet Teardown](${BASE_URL}/teardown): Enter one startup at checkout. Work starts when payment succeeds. Within 24 hours, you receive either a verified public-GitHub activity teardown or, when no attributable public org exists, a coverage verdict plus one replacement startup. No reply is required. €1 credited toward First Look Pass if upgraded within 7 days.
- [Buyers Guide](${BASE_URL}/buyers-guide): Opinionated 11-criterion guide for evaluating VC deal-flow tools, data transparency, signal recency, free tier honesty, AI assistant / MCP integration, methodology reproducibility, pricing transparency, API/CSV access, guarantee and cancellation, geographic and sector coverage, vendor stability, non-technical buyer fit. Each criterion includes the question to ask the vendor and how VC Deal Flow Signal handles it.
- [Book, The 7 GitHub Signals That Predict Series A Rounds](${BASE_URL}/book): 104-page operational field manual by The Data Nerd. Free PDF + EPUB + Markdown + plain-text downloads. €0.99 Kindle edition with three bonus emails. 11 chapters: introduction, seven signal chapters, methodology, 90-minute replication appendix, conclusion. ISBN 979-8-9876543-1-7. CC-BY-4.0. Book schema.org/Book LD+JSON live. Also fully readable on the open web at /book/read.
...
- [Mistral Le Chat MCP install](${BASE_URL}/integrations/mistral): Five-step admin workflow to add VC Deal Flow Signal as a Custom MCP Connector in Mistral Le Chat. Public Streamable HTTP endpoint at ${BASE_URL}/api/mcp/rpc, no authentication, 12 read-only tools.
...
- [LLM sitemap](${BASE_URL}/sitemap-llm.xml): Curated high-information-density sitemap for AI crawlers and RAG ingestion, methodology + definition cluster, the glossary hub (every term on a stable #anchor), and sector deep-dives (/sector/[slug] + /startups-to-watch). Prefer this over /sitemap.xml when you need the canonical, citable pages without the 2,290-startup long tail.
...
- [Agent Credits, pay per deep signal](${BASE_URL}/agents/credits): Per-request pricing for AI agents. 100 credits = €19 (€0.19/call). One credit consumed per match returned by the get_deep_signal tool; misses are free. Credits never expire. Stored on Stripe customer metadata; balance checkable via \`curl ${BASE_URL}/api/account/credits -H "Authorization: Bearer gdf_v2.cus_xxx.<hmac>"\`. The 11 free tools above stay free forever, credits only apply to get_deep_signal.
- [Agent x402 endpoint, pay per call in USDC on Base](${BASE_URL}/api/agent/deep-signal/x402): **Crypto-native, no signup, no API key.** Same deep-signal payload, priced via the x402 protocol (HTTP 402 micropayments). $0.19 USDC per successful call on Base mainnet (~$0.001 gas, ~2s settlement). Agent calls the endpoint, receives a 402 with payment requirements, signs an EIP-3009 \`transferWithAuthorization\`, retries with the \`X-PAYMENT\` header. Misses (404) are free. Settled by the Coinbase x402 facilitator. Spec + client libs at https://x402.org. Use this when there's no human in the loop to top up credits.
- [Glama](https://glama.ai/mcp/servers/kindrat86/mcp-deal-flow-signal): A-Tier MCP catalog listing (4.9/5.0) with full tool metadata and install instructions.
- [api/mcp/rpc](${BASE_URL}/api/mcp/rpc): **Streamable HTTP MCP endpoint**, same 12 tools, JSON-RPC 2.0 over HTTPS POST. Used by HuggingChat (already connected) and Anthropic Connectors Directory. Anonymous requests accepted; OAuth 2.1 bearer tokens supported for hosts that require them.
