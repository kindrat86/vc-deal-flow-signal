...
import "./globals.css";
// Load-bearing subset of public/ux.css (generated by scripts/gen-critical-css.mjs).
// Bundled into the render-blocking chunk so first paint needs no extra CSS
// round trip; the full /ux.css loads asynchronously (FCP fix 2026-08-16).
import "./critical.css";
  ...
  return (
    <html lang="en" className="h-full">
      <head>
        ...
        {/* R16/R18 UX layer: load-bearing base rules ship in app/critical.css
            (bundled into the render-blocking chunk above), so the full file
            can load asynchronously and stop blocking first paint
            (FCP fix 2026-08-16). media="print" is non-blocking; the inline
            script swaps it to "all" on load; noscript covers no-JS clients.
            Versioned so in-app browsers cannot reuse a stale palette after
            deployment. */}
        <link id="ux-css" rel="stylesheet" href="/ux.css?v=20260816-1" media="print" />
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){var l=document.getElementById("ux-css");if(!l){return;}function on(){l.media="all";}try{if(l.sheet&&l.sheet.cssRules){on();return;}}catch(e){}l.addEventListener("load",on);})();`,
          }}
        />
        <noscript>
          <link rel="stylesheet" href="/ux.css?v=20260816-1" />
        </noscript>
