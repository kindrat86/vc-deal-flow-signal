import { ImageResponse } from "next/og";
import { parseTrendSlug } from "@/lib/data";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "VC Deal Flow Signal — Trend";

export default async function OGImage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const t = parseTrendSlug(slug);

  const headline = t
    ? `${t.sector.name}: ${t.periodA.name} vs ${t.periodB.name}`
    : "VC Deal Flow Signal";
  const subhead = t
    ? `Period-over-period engineering acceleration across ${t.sector.name.toLowerCase()} startups.`
    : "";

  return new ImageResponse(
    (
      <div
        style={{
          display: "flex",
          flexDirection: "column",
          width: "100%",
          height: "100%",
          backgroundColor: "#0f172a",
          color: "#f1f5f9",
          padding: "60px",
        }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px" }}>
          <div style={{ display: "flex", fontSize: 20, color: "#0ea5e9", fontWeight: 600 }}>
            VC Deal Flow Signal
          </div>
          <div style={{ display: "flex", fontSize: 18, color: "#64748b" }}>
            Trend · period-over-period
          </div>
        </div>
        <div style={{ display: "flex", fontSize: 52, fontWeight: 700, lineHeight: 1.15, marginBottom: "20px" }}>
          {headline}
        </div>
        <div style={{ display: "flex", fontSize: 22, color: "#94a3b8", lineHeight: 1.4, maxWidth: "900px", marginBottom: "20px" }}>
          {subhead}
        </div>
        {t && (
          <div style={{ display: "flex", gap: "60px", alignItems: "baseline" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", fontSize: 28, fontWeight: 700, color: "#94a3b8" }}>{t.periodA.name}</div>
              <div style={{ display: "flex", fontSize: 16, color: "#64748b" }}>baseline</div>
            </div>
            <div style={{ display: "flex", fontSize: 28, color: "#0ea5e9" }}>→</div>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", fontSize: 28, fontWeight: 700, color: "#0ea5e9" }}>{t.periodB.name}</div>
              <div style={{ display: "flex", fontSize: 16, color: "#64748b" }}>compared</div>
            </div>
          </div>
        )}
        <div style={{ display: "flex", marginTop: "auto", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", fontSize: 16, color: "#475569" }}>
            signals.gitdealflow.com/trends/{slug}
          </div>
          <div style={{ display: "flex", fontSize: 16, color: "#475569" }}>CC BY 4.0</div>
        </div>
      </div>
    ),
    size
  );
}
