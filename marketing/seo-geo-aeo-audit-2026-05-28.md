...
| 3 | ✅ **Analyzed (this PR):** see `pseo-thin-page-analysis-2026-05-28.md`. Finding: the combinatorial crossings are **already** gated by `MIN_PSEO_CELL_SIZE=3` (stage×sector, signal×sector, startup-cohort) and by real HQ/editorial data (sector×city) — the exact mechanism this rec asked for. Residual risk is narrowed to curation-gated editorial corpora (niche-down, comparisons, from-stars-to-seed). Next safe step: a read-only build-time thinness probe before any `noindex`/prune. **No index changes made.** | De-risks scaled-content penalty; protects pSEO 96 | M |
