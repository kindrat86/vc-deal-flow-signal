...
export function auditPublicClaims(root = process.cwd()) {
  ...
  const homepage = pages.get(join(root, 'index.html')) ?? '';
  ...
  for (const needle of homepageRequired) {
    if (!homepage.includes(needle)) failures.push(`homepage missing corrected claim: ${needle}`);
  }
  ...
  for (const [file, html] of pages) {
    for (const [label, pattern] of PUBLIC_CLAIM_BANS) {
      if (pattern.test(html)) failures.push(`${file.slice(root.length + 1)} contains banned public claim: ${label}`);
    }
  }
