 ...
 */
export async function runAgentCycle(type) {
    ...
    try {
        ...
        console.log(`[agent] Diff: ${diff.newStartups.length} new, ${diff.signalTypeChanges.length} type changes, ` +
        ...
        // ── Stage 6: Save ─────────────────────────────────────────────────
        const savedBriefId = await memory.saveBrief(brief, type, type === "weekly" ? "pending_approval" : "draft");
        ...
        const shouldPublish = type === "weekly" ? autoPublish :
        ...
        if (shouldPublish) {
            ...
            await memory.createApproval("publish_brief", savedBriefId, "brief", `Weekly brief ready: "${brief.title}" — ${brief.highlights.length} highlights`, { period: brief.period });
        }
        // ── Stage 8: Complete ─────────────────────────────────────────────
...
async function main() {
    ...
    if (onceMode) {
        ...
        const type = args.includes("--midweek") ? "midweek" : "manual";
