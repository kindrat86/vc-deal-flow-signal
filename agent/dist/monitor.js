...
export function flattenStartups(data) {
    const seen = new Set();
    const rows = [];
...
// ─── Diff Detection ────────────────────────────────────────────────────
export async function computeDiff(current) {
    ...
    for (const name of currNames) {
        if (!prevNames.has(name)) {
            ...
            if (entry)
                newStartups.push(entry.startup);
        }
    }
    // Removed startups (partial info — only name is reliable)
    const removedStartups = [];
    ...
    console.log(`[monitor] Diff: ${newStartups.length} new, ${removedStartups.length} removed, ` +
...
// ─── Breakout Alert ────────────────────────────────────────────────────
export function detectBreakouts(diff, startups) {
    const breakouts = [];
    // New startups that are already scoring high
    for (const s of startups.filter((startup) => diff.newStartups.some((ns) => ns.name === startup.name))) {
        const scored = analyst.scoreStartup(s);
    ...
    for (const change of diff.signalTypeChanges) {
        if (change.new.toLowerCase().includes("breakout") ||
