...
import type { Startup, SignalScore, ScoreBreakdown, EnrichedStartup } from "./types.js";
 ...
 */
export function scoreStartup(s: Startup): SignalScore {
  ...
  const raiseLikelihood =
    total >= 70 ? "high" as const :
    total >= 45 ? "elevated" as const :
    total >= 25 ? "moderate" as const :
    "low" as const;

  const estimatedWindow =
    raiseLikelihood === "high" ? "3-6 months (heuristic)" :
...
export function classifyTier(score: SignalScore): SignalTier {
  if (score.accelerationScore >= 70) return "breakout";
  if (score.accelerationScore >= 45) return "accelerating";
  if (score.accelerationScore >= 25) return "steady";
  return "cooling";
}
...
export async function enrichStartups(
  ...
  const enriched: EnrichedStartup[] = startups.map((s) => {
...
export function rankByScore(
  enriched: EnrichedStartup[]
): EnrichedStartup[] {
  return [...enriched].sort((a, b) => b.score.accelerationScore - a.score.accelerationScore);
}

export function rankBySector(
  enriched: EnrichedStartup[]
): Map<string, EnrichedStartup[]> {
  const sectors = new Map<string, EnrichedStartup[]>();
...
const DEFAULT_CRITERIA: HighlightCriteria = {
  maxCount: 5,
  minScore: 25,
  preferNew: true,
  preferSurging: true,
...
export function selectHighlights(
  enriched: EnrichedStartup[],
  criteria: Partial<HighlightCriteria> = {}
): EnrichedStartup[] {
  ...
  const selected: EnrichedStartup[] = [];
  const seenSectors = new Set<string>();

  // First pass: take breakouts
  for (const s of breakouts) {
    if (selected.length >= c.maxCount) break;
    if (c.diversityAcrossSectors && seenSectors.has(s.sectorSlug)) continue;
    selected.push(s);
  ...
  if (selected.length < c.maxCount) {
    const remaining = [...breakouts, ...topScorers].filter(
...
export function generateSummary(enriched: EnrichedStartup[], highlights: EnrichedStartup[]): string {
