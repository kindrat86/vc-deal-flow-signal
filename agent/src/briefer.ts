...
function templateNarrative(s: EnrichedStartup): string {
...
async function llmNarrative(s: EnrichedStartup): Promise<string | null> {
...
export async function buildWeeklyBrief(
  ...
  const briefs: StartupBrief[] = [];
  for (let i = 0; i < highlights.length; i++) {
    if (!guardrails.trackBriefGenerated()) break;
...
export function renderEmailHtml(brief: WeeklyBrief): string {
...
export function renderTelegramMd(brief: WeeklyBrief): string {
