...
app.get("/api/preview", async (_req, res) => {
  try {
    ...
    const brief = await buildWeeklyBrief(
      signals.map((s) => ({ ...s, score: s.score, sectorName: s.sectorName, sectorSlug: s.sectorSlug })),
      "Preview of this week's top signals.",
      "Preview",
      false
    );
...
export function startServer(port: number = 3400): void {
  app.listen(port, () => {
