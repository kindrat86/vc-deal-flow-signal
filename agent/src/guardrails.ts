...
import type { WeeklyBrief, StartupBrief } from "./types.js";
...
export function assessRisk(action: string, context: Record<string, unknown> = {}): RiskAssessment {
...
export function validateBrief(brief: WeeklyBrief): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  for (const h of (brief.highlights || [])) {
    if (!h.name) issues.push(`Highlight missing name`);
    if (!h.githubUrl) issues.push(`${h.name}: missing GitHub URL`);
    if (!h.narrative) issues.push(`${h.name}: missing narrative`);
    if (!h.signalType) issues.push(`${h.name}: missing signal type`);
  }
...
export function validateDisclaimer(brief: WeeklyBrief): boolean {
...
export async function checkApprovalGate(
  ...
  const approvalId = await memory.createApproval(
    action,
    0,
    "action",
    `Approval required: ${action} — ${risk.reason}`,
    context
  );
...
export function sanitizeForPublishing(text: string): string {
  return text
    ...
    .replace(/\b[A-Z][a-z]+ [A-Z][a-z]+\b/g, (match) => {
      ...
      if (safe.some((s) => match.includes(s))) return match;
...
export function trackLlmCall(): boolean {
  cycleLlmCalls++;
...
export function trackBriefGenerated(): boolean {
...
export function getCycleStats(): { llmCalls: number; briefsGenerated: number } {
